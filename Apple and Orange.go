...
func countApplesAndOranges(s int32, t int32, a int32, b int32, apples []int32, oranges []int32) {
	var finalAppleDistance  []int32
	var finalOrangeDistance []int32
	for _, v := range apples {
		v += a
		finalAppleDistance = append(finalAppleDistance, v)
	}
	for _, v := range oranges {
		v += b
		finalOrangeDistance = append(finalOrangeDistance, v)
	}
	var countApples int32 = 0
	var countOranges int32 = 0
	for _, v := range finalAppleDistance {
		if v >= s && v <= t {
			countApples +=1
		}
	}
	for _, v := range finalOrangeDistance {
		if v >= s && v <= t {
			countOranges +=1
		}
	}
